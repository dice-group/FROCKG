...
import {EventProviderService} from "../../service/event-provider.service";
...
export class GraphComponent implements OnInit {
  ...
  constructor(public uip: UniqueIdProviderService, public eventService: EventProviderService) {}
  ngOnInit() {
    console.log('this is graph view data');
  ...
  fillGraph() {
        ...
        this.sNodeX = width / 2;
        this.getDefaultPath(this.graphData.fact);
        this.getAllPaths(this.graphData.piecesOfEvidence);
    ...
    svg.append('svg:defs').append('svg:marker')
      .attr('id', 'arrowInverted').attr('viewBox', '0 0 10 10')
      .attr('refX', 0).attr('refY', 5)
      .attr('markerUnits', 'strokeWidth')
      .attr('markerWidth', 7)
      .attr('markerHeight', 2.5)
  ...
  getMaxDist(data: CgData) {
    ...
    for (const x in this.graphData.piecesOfEvidence) {
      const curLen = this.graphData.piecesOfEvidence[x].evidence.length;
      if (curLen > maxLen) {
        maxLen = curLen;
      }
    }
  ...
  getDefaultPath(triple: CgTriple) {
    ...
    this.eNode = eNode;
    const edge = new CgLineItem(sNode.cx, sNode.cy, eNode.cx, eNode.cy, triple.property, 0.5, this.uip.getUniqueId(), -1,false);
  ...
  drawEdgeLables(items: CgLineItem[]) {
    ...
    const labelEnter = label.enter().append('text').attr('dy', -10).attr('transform', function(d) {
      ...
      .attr('startOffset', '50%')
      .text(function(d) { return GraphComponent.getUriName(d.uri); })
      .attr('class', 'edge-lbl')
    ;
  ...
  drawEdges(items: CgLineItem[]) {
    ...
    const lines = linesG.append('path')
      ...
      .attr('inverted', function(d) { return d.inverted; })
      .attr('d', this.getEdgePath)
      .style('fill', 'none')
      .attr('class', 'line-def ')
      .style('stroke-width', function(d) { return (d.pathScore + 2) * curScope.pathStrFac + 'px'; })
      .attr('marker-mid', function(d) {if(!d.inverted){return 'url(#arrow)'}else{return 'url(#arrowInverted)'}})
  ...
  sendDetails(cgPaths: CgPath[]) {
    this.eventService.sendDetailEvent.emit(cgPaths);
  }
  ...
  getAllPaths(cgPaths: CgPath[]) {
    ...
    for (let i = 0; i < cgPaths.length; i++) {
      ...
      const xDelta = this.maxXDist / curPath.evidence.length;
      const pathScore = curPath.score;
      // Loop through each triple in a path
      for (let j = 0; j < curPath.evidence.length; j++) {
        const curTriple = curPath.evidence[j];
        ...
        const edge: CgLineItem = new CgLineItem(0, 0, 0, 0, curProp, pathScore, this.uip.getUniqueId(), pathId, inverted);
        let fromNode: CgNodeItem;
        let toNode: CgNodeItem;
        console.log('curTriple : '+curTriple);
        console.log('curSub : '+curSub);
        console.log('curObj : '+curObj);
        console.log('prevNode.uri : '+prevNode.uri);
        console.log('----- ');
